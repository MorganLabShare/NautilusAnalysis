...
obMovDir = [WPN '\movies\IxQ\cid4_plusBips3\'];
...
groupId = [repmat(1,[length(group1) 1]);repmat(2,[length(group2) 1]);repmat(3,[length(group3) 1]);...
    repmat(4,[length(group4) 1]); repmat(5,[length(group5) 1]); repmat(6,[length(group6) 1])];
...
for i = 1:length(colorGroup)
    colorNum = sum(groupId==colorGroup(i));
   eval(sprintf('cmap = %s(%d);',colorMapGroup{i},colorNum)); 
   col(groupId==colorGroup(i),:) = cmap;
end
