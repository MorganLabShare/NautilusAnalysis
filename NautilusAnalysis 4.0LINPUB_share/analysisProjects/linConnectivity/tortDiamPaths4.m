...
goodPath = (fac'>0) & (L>2) & ~isnan(pTort) & ~isnan(pDiam) & ~isnan(pVar);

source(p) = np;
L = L(goodPath);
pTort = pTort(goodPath);
pDiam = pDiam(goodPath);
pTarg = pTarg(goodPath);
pShaft = pShaft(goodPath);
pAx = pAx(goodPath);
pVar = pVar(goodPath);

isShaftPath =   find(((pShaft./L)>.9));
isTargPath = find(((pTarg./L)>.9));
...
%nFac = fac>=5;%(fac-mean(fac))/std(fac) * 1;
% 
...
filename = 'C:\Users\jlmorgan\Documents\Publications\LIN\Revision\Pics\bimod.eps';
 print -depsc -tiff -r300 -painters C:\Users\jlmorgan\Documents\Publications\LIN\Revision\Pics\bimod.eps
