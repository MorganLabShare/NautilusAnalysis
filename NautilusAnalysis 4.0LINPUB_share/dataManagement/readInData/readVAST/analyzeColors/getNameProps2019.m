...
for i = 1:length(objNames)  
    ...
    tag.soma(i) = sum(regexp(nam,'soma')>0);
...
nameProps.ofID = tag.init |tag.brh |tag.frag | tag.part | tag.cell | tag.glom | tag.center | tag.axprim |tag.retZone | tag.distal | tag.bout;
nameProps.tag = tag;
